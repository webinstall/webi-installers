...
fn_node_install() { (
    ...
    if test -n "${my_path_actual}"; then
        ...
        if test ! -w "${my_bin_dir}" ||
            ...
            printf "\e[31m[Error]\e[0m \e[32mNode.js\e[0m at %s is not user-writable\n" "${my_path_actual}"
            sleep 1
    ...
    if test -n "${my_path_actual}"; then
        ...
        if test -z "${my_node_major}"; then
            ...
            printf "\e[31m[Error]\e[0m \e[32mNode.js v%s\e[0m is too old\n" "${my_node_version}"
            sleep 1
...
fn_fish_install() { (
    if ! command -v fish > /dev/null; then
        printf "\e[31m[Error]\e[0m \e[32mfish\e[0m not found\n"
